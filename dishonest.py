...
if __name__ == '__main__':
    ...
    global network
    network = Network( 4 )
    network.add_node(65432) #generator
    ...
    #honest block
    ignore  = (listen_broadcast(s))
    print("although block accepted and valid, it is ignored")

    dishonest_block = generate(network.chain[0])
    network.add_block(dishonest_block)
    time.sleep(.5)
    broadcast(dishonest_block)

    #wont work, from honest after addition of dishonest
    network.add_block(listen_broadcast(s))
